...
#| label: wd-3-date
#Lab 5
#Revised to mutate date into a date object and use mdy function
crime_scene_report |>
...
trout_likely <- trout_cm |>
  mutate(length = replace_unlikely(length, 5, 80),
         weight = replace_unlikely(weight, 10, 4000))
```
...
#| label: wd-6-anti
#Lab 4 Q5
#Modified to use an anti join instead of a filter to get a dataset with only the years 2008 and 2018.
anti <- ca_childcare |>
...
ca_childcare |>
anti_join(y = anti,
          by = join_by(study_year == study_year)) |>
pivot_wider(names_from = study_year,
            values_from = mhi_2018) |> 
group_by(region) |>
summarize(`2008 median household income` = median(`2008`, na.rm = TRUE),
...
rescale_01 <- function(x) {
   if (!is.numeric(x))  return(NA)
   if (!length(x) > 1) return(NA) #https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/length
   rge <- range(x, na.rm = TRUE) #https://r4ds.hadley.nz/functions#writing-a-function
   (x - rge[1]) / (rge[2] - rge[1])
}
...
#Wouldn't it be nice to know where the measurements for each year were? You could do this by forcing year to have levels where there were no observations or by converting it to categorical! How could you use yout plot title to convey additional information regarding the context of these data?

#Modified to change year to a factor and have a more descriptive title
...
theme(legend.position = "none") #https://r-graph-gallery.com/239-custom-layout-legend-ggplot2.html
```
...
#Lab 10 Q13
y = 1:100
colors = c("#99afd7", "#ff0000")
set.seed(123)
table |>
  mutate(ci = if_else(conf.high < 0.5 | conf.low > 0.5, "out","in")) |>
  slice_sample(n = 100, replace = FALSE) |>
  ggplot(mapping = aes(y, x = estimate, color = ci)) +
  geom_errorbar(aes(xmin = conf.low, 
                    xmax = conf.high)) +
  theme_bw() +
  geom_vline(xintercept = 0.5, linetype = "dashed", colour = "steelblue") +
  annotate("text", x = 1, y = 104, label="Population slope = 0.5") + #https://ggplot2.tidyverse.org/reference/annotate.html
  labs(y = " ",
       x = "Slope Estimate",
       title = "95% Confidence Intervals for Estimated Slope of 100 Random Simulations",
       subtitle = "Confidence intervals not containing the population slope shown in <span style = 'color:#FF0000;'>red") +
  scale_color_manual(values = colors) +
  theme(legend.position = "none",
...
#| label: dvs-6-ex-2
#Lab 9 Q6
#We have not learned the paste() or paste0() functions in this class! The str_c() function is the tool we have learned. Can you convert your solution to include the tool we’ve learned in class?
#Modified to use stringr 

unformatted_evals |>
  mutate(prop = str_c(round(prop *100, 2), "%")) |>  
  kable(col.names = c("Demographic",
                           "Group",
                           "Count",
                           "%"),
        caption = "Demographics of Professors at the University of Poland, Warsaw in 2020") |> #https://stackoverflow.com/questions/51432502/replace-column-names-in-kable-r-markdown
  kable_styling(full_width = FALSE,
                latex_options = "basic") |>
  add_header_above(c(" " = 2, "Professor" = 2)) |> #https://bookdown.org/yihui/rmarkdown-cookbook/kableextra.html
...
fish |>
  ...
  tab_header(title = md("**Number of Missing Values for Fish Measurements**"),
             subtitle = "Data collected from 1989 to 2006 on the Blackfoot River in Montana") |>
  data_color(method = "numeric",
             palette = c("#008080", "#be5103")) 
...
unformatted_evals <- evals |>
  ...
  mutate(academic_degree = str_replace_all(academic_degree, c("dr" = "Doctorate",
         ...
         sex = str_replace_all(sex, c("female" = "Female",
                                      "^male" = "Male")),
         seniority = case_when(seniority <= 4 ~ "Junior",
                             seniority > 4 & seniority < 9 ~ "Senior",
                             seniority > 8 ~ "Very Senior")) |>
  rename(`Academic Degree` = academic_degree,
         Seniority = seniority,
         Sex = sex) |>
...
#| label: pe-2-2
#Lab 8 Q3
#It looks like you are doing the same process to both of your axis labels. Time for a helper function!
#Revised to add the make_title function which I then can call inside the boxplot function to make my code more efficient
make_title <- function(label){
  title <- as_label(ensym(label)) |>
    str_replace_all(pattern ="_", replacement = " ") |>
    str_to_sentence() 
  title
}

boxplot <- function(df, cat_var, num_var){

  x_title <- make_title({{cat_var}})
  y_title <- make_title({{num_var}})
  label <- rlang::englue("Boxplot of {y_title} across {x_title}")
  
  df|>
    ggplot(mapping = aes(x = {{cat_var}}, y= {{num_var}}, fill = {{cat_var}})) +
    geom_boxplot() +
    labs(x = x_title,
         y = "",
         title = label) +
...
#Modified to add function checks to ensure input is numeric and of length 1
...
mycifun <- function(beta0, beta1, n){
  ...
  broom::tidy(model2, 
            ...
            conf.level = 0.95) |>
  filter(term == "x") |>
  mutate(cover = if_else(beta1 > conf.low & 
                          beta1 < conf.high, 
                          1, 
                          0)) |>
...
#Revised to not save an a=object
